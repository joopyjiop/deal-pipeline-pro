  ...
  const checkConnection = async () => {
    ...
    try {
      ...
      if (mode === "CLOUD") {
        ...
        setAvailableModels(models);
        setConnectionState("connected");
        toast.success(models.length ? `AI gateway connected (${models.length} model${models.length === 1 ? "" : "s"}).` : "AI gateway connected.");
        return;
      }
      ...
      toast.error(error instanceof Error ? error.message : mode === "CLOUD" ? "Could not reach the AI gateway." : "Could not reach the selected agent connection.");
    }
  };
  ...
  const runAgent = async () => {
    ...
    try {
      ...
      const runCloud = async () => {
        ...
        const content = extractContent(payload);
        if (!content) throw new Error("AI gateway returned no text");
        return content;
      };
