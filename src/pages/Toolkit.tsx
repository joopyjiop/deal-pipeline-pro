...
export default function Toolkit() {
  ...
  const [crawling, setCrawling] = useState(false); const [showAllPages, setShowAllPages] = useState(false);
      ...
      <div className="toolkit-container mx-auto max-w-[1500px]" ref={toolkitRef}>
        ...
        <section className="glass-panel mt-5 rounded-[1.75rem] p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-start gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-sky-100/80 text-sky-700"><Search className="size-5" /></div><div><p className="eyebrow">Semantic lead search</p><h2 className="mt-1 text-lg font-semibold tracking-tight text-slate-900">Find leads by meaning, not just keywords</h2><p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">Embeddings run through your AI gateway (AI_BASE_URL, default local OmniRoute) with text-embedding-3-small and rank every indexed lead by cosine similarity against your query. Only non-fabricated leads are indexed; nothing leaves the pipeline except the query text. Set AI_BASE_URL (and AI_API_KEY if your gateway expects one) in the Convex Keys panel to enable indexing.</p></div></div>
