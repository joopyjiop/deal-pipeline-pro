...
- **Signed-in non-owners:** the **read-only marketplace** (`src/convex/marketplace.ts` + `MarketplaceView.tsx`) — approved sellers (verified, non-fabricated leads with PII scrubbed), approved buyers (contact PII stripped), and matches. No write paths, no owner buttons, no lead dossiers, no owner workspace. Guest/anonymous accounts are not offered and are rejected server-side. Sellers sortable by **best deals** (distress), **most buyers** (active match count per deal), and **most profit** (estimated spread = MAO − contract price) via the shared `sortDeals` helper (`marketplaceCore.ts`); the same sort lives on the owner's Dashboard lead queue (`/dashboard`), which loads owner-only match counts from `listMatches` to power it.
...
| `import_staging` | Pending source/staging queue: `sourceType`, `rawJson`, `status` (`NEW`\|`NEEDS_EVIDENCE`\|`DUPLICATE`\|`REJECTED`\|`ARCHIVED`), `sourceUrl?`, `sourceRef?`, `sourceDate?`, `distressScore?`, `missingEvidence?`, `rejectReason?`, `aiCourtVerdict?`, `candidateLeadId?`, `createdAt`, `updatedAt`. A row written without complete source evidence is auto-flagged `NEEDS_EVIDENCE` (never `NEW`). Public buyer intake also lands here as `listSource: "PUBLIC_INTAKE"`. Rows with no source URL anywhere and no readable content are empty garbage and are deleted by `stagingCleanup` (owner button in Operations + daily cron) |
