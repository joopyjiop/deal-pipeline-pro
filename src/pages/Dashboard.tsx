...
export default function Dashboard() {
  ...
  if (!isOwner) {
    return (
      <main className="admin-dashboard min-h-screen px-4 py-6 text-foreground sm:px-6 lg:px-8">
  ...
  return (
    <main className="admin-dashboard min-h-screen px-4 py-6 text-foreground sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-[1500px] gap-6">
        <aside className="admin-sidebar hidden w-64 shrink-0 flex-col rounded-2xl border-0 bg-slate-950 p-4 text-white shadow-xl lg:flex">
      ...
      {showAdminTools && isOwner && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/15 p-3 backdrop-blur-sm sm:items-center" onClick={(event) => { if (event.target === event.currentTarget) setShowAdminTools(false); }}>
          <div className="admin-tools-modal w-full max-w-4xl overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl sm:p-0">
            <div className="admin-tools-hero relative overflow-hidden bg-slate-950 px-5 py-6 text-white sm:px-8 sm:py-8">
              <div className="relative z-10 flex items-start justify-between gap-5">
                <div>
                  <div className="flex items-center gap-2 text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-indigo-300"><Wrench className="size-3.5" /> Owner command center</div>
                  <h2 className="mt-3 text-3xl font-semibold tracking-tight">Admin tools</h2>
                  <p className="mt-2 max-w-xl text-sm leading-6 text-slate-300">Run access, automation, and data operations from one controlled workspace.</p>
                </div>
                <button type="button" onClick={() => setShowAdminTools(false)} className="flex size-9 shrink-0 items-center justify-center rounded-xl border border-white/15 bg-white/10 text-slate-300 transition-colors hover:bg-white/15 hover:text-white" aria-label="Close admin tools"><X className="size-4" /></button>
              </div>
              <div className="relative z-10 mt-7 grid grid-cols-3 gap-2 sm:max-w-md sm:gap-3">
                <div className="rounded-xl border border-white/10 bg-white/10 px-3 py-2.5"><p className="text-[0.6rem] font-semibold uppercase tracking-wider text-slate-400">Workspace</p><p className="mt-1 text-sm font-semibold text-white">Owner only</p></div>
                <div className="rounded-xl border border-white/10 bg-white/10 px-3 py-2.5"><p className="text-[0.6rem] font-semibold uppercase tracking-wider text-slate-400">Lead origin</p><p className="mt-1 text-sm font-semibold text-teal-300">Verified</p></div>
                <div className="rounded-xl border border-white/10 bg-white/10 px-3 py-2.5"><p className="text-[0.6rem] font-semibold uppercase tracking-wider text-slate-400">Guardrails</p><p className="mt-1 text-sm font-semibold text-indigo-300">Enabled</p></div>
              </div>
              <div className="admin-tools-orb absolute -right-16 -top-28 size-72 rounded-full bg-indigo-600/25 blur-3xl" />
              <div className="admin-tools-orb absolute -bottom-40 left-1/2 size-72 rounded-full bg-teal-500/10 blur-3xl" />
            </div>
            <div className="p-5 sm:p-8">
              <div className="mb-4 flex items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-indigo-600">Workspace modules</p><p className="mt-1 text-sm text-slate-500">Select a tool to open its full workflow.</p></div><span className="hidden text-xs font-medium text-slate-400 sm:block">5 owner actions</span></div>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <button type="button" onClick={() => { setShowAdminTools(false); setShowApiAccess(true); void loadApiCredentials(); }} className="admin-tools-card group rounded-xl border border-slate-200 bg-slate-50 p-5 text-left transition-colors hover:border-sky-300 hover:bg-sky-50/70">
              ...
              <button type="button" onClick={() => { setShowAdminTools(false); setShowUsers(true); void handleLoadUsers(); }} className="admin-tools-card group rounded-xl border border-slate-200 bg-slate-50 p-5 text-left transition-colors hover:border-violet-300 hover:bg-violet-50/70">
              ...
              <button type="button" onClick={() => { setShowAdminTools(false); setShowAiUsage(true); void loadAiUsage(); }} className="admin-tools-card group rounded-xl border border-slate-200 bg-slate-50 p-5 text-left transition-colors hover:border-teal-300 hover:bg-teal-50/70">
              ...
              <button type="button" onClick={() => { setShowAdminTools(false); setShowFileUpload(true); }} className="admin-tools-card group rounded-xl border border-slate-200 bg-slate-50 p-5 text-left transition-colors hover:border-emerald-300 hover:bg-emerald-50/70">
      ...
      {showAiUsage && isOwner && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/15 p-3 backdrop-blur-sm sm:items-center" onClick={(event) => { if (event.target === event.currentTarget) setShowAiUsage(false); }}>
          <div className="rounded-xl border border-slate-200 bg-white shadow-xl max-h-[94vh] w-full max-w-2xl overflow-y-auto p-5 sm:p-7">
