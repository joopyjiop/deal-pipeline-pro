      ...
      <div className="mx-auto max-w-6xl">
        <header className="flex items-center justify-between py-3">
          <Link to="/" className="flex items-center gap-3" aria-label="Groundwork home">
            <span className="flex size-10 items-center justify-center rounded-2xl border border-white/90 bg-white/70 text-sky-700 shadow-sm backdrop-blur-xl"><Landmark className="size-5" /></span>
            <span className="text-base font-bold tracking-tight text-slate-800">Groundwork</span>
          </Link>
          <Link to="/" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 transition-colors hover:text-sky-700"><ArrowLeft className="size-4" /> Back home</Link>
        </header>
          ...
          <div className="glass-panel-strong rounded-[2rem] p-5 sm:p-8">
            {submitted ? (
              <div className="flex min-h-[420px] flex-col items-center justify-center px-5 text-center">
                <div className="flex size-16 items-center justify-center rounded-3xl bg-teal-100/80 text-teal-700"><CheckCircle2 className="size-8" /></div>
                ...
                <Link to="/" className="mt-7 inline-flex rounded-full bg-sky-700 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-sky-800">Return to Groundwork</Link>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="grid gap-5">
                ...
                <div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Your buy box</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><Input required type="number" min="0" value={form.budgetMin} onChange={(event) => update("budgetMin", event.target.value)} placeholder="Minimum purchase budget" className="rounded-xl border-white/85 bg-white/65" /><Input required type="number" min="0" value={form.budgetMax} onChange={(event) => update("budgetMax", event.target.value)} placeholder="Maximum purchase budget" className="rounded-xl border-white/85 bg-white/65" /><Input required value={form.targetAreas} onChange={(event) => update("targetAreas", event.target.value)} placeholder="Target areas, separated by commas" className="rounded-xl border-white/85 bg-white/65 sm:col-span-2" /><label className="grid gap-1.5 text-xs font-semibold text-slate-600 sm:col-span-2"><span>Exit strategy</span><select value={form.exitType} onChange={(event) => update("exitType", event.target.value)} className="h-10 rounded-xl border border-white/85 bg-white/70 px-3 text-sm font-medium text-slate-800 outline-none focus:ring-2 focus:ring-sky-500/30"><option value="ASSIGN">Assign the contract</option><option value="FLIP">Fix and flip</option><option value="BUY_HOLD">Buy and hold</option></select></label></div></div>
                <div className="rounded-2xl border border-sky-100/90 bg-sky-50/55 p-4 text-xs leading-5 text-slate-600">By submitting, you’re asking to be considered as a buyer. Your profile will remain <strong className="text-slate-800">pending + unverified</strong> until the owner reviews it.</div>
                <Button type="submit" disabled={isSubmitting} className="h-11 rounded-xl bg-sky-700 text-sm font-semibold hover:bg-sky-800">{isSubmitting ? <><Loader2 className="size-4 animate-spin" /> Submitting</> : "Submit buyer profile"}</Button>
              </form>
            )}
          </div>
        </section>
